...
describe('ANNOTATION_GEOMETRY', () => {
  it('box: createFromDragはドラッグ矩形からwidth/heightを計算する', () => {
    const created = ANNOTATION_GEOMETRY.box.createFromDrag(
      1,
      { x: 10, y: 20 },
      { x: 30, y: 50 },
      boxStyle,
  ...
  it('line: createFromDragはstartを起点としたpointsを生成する', () => {
    const created = ANNOTATION_GEOMETRY.line.createFromDrag(
      1,
      { x: 5, y: 5 },
      { x: 15, y: 25 },
      lineStyle,
  ...
  it('circle: createFromDragはドラッグの中点と半径を計算する', () => {
    const created = ANNOTATION_GEOMETRY.circle.createFromDrag(
      1,
      { x: 0, y: 0 },
      { x: 6, y: 8 },
      circleStyle,
  ...
  it('arrow: createFromDragはlineと同じpoints規約に矢じり設定を加えて生成する', () => {
    const created = ANNOTATION_GEOMETRY.arrow.createFromDrag(
      1,
      { x: 0, y: 0 },
      { x: 10, y: 0 },
      arrowStyle,
